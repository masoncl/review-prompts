...
1. Reading every reply from the maintainer teammates (and Linus, if
   involved).
2. Classifying each comment (accept / reject / defer).
3. Applying the accepted feedback to the working tree (unless
   `--no-apply` was passed in the spawn prompt).
4. Regenerating the patch series as v<next> with a changelog cover letter.
...
4. **Outputs stay local.** Revised patches live under
   `./kmail-review/v<next>-patches/`. Nothing is delivered anywhere.
...
`maintainer-cross-review` (and `review-linus` if it exists). Do not
start until all blockers are `completed`.

### Step 2 — Read everything

Read, in order:
1. Your original commit messages via `./kmail-review/context/commit-message.json`.
2. The diff: `./kmail-review/context/change.diff`.
3. The original patches: `./kmail-review/patches/*.patch`.
4. Every maintainer reply: `./kmail-review/thread/01-maintainer-*.txt`.
5. Any cross-review follow-ups: `./kmail-review/thread/01-maintainer-*-crossreview.txt`.
6. Linus's reply (if present): `./kmail-review/thread/02-linus.txt`.
...
1. Read `./kmail-review/context/version.json` for the version numbers.
2. Create a scratch branch for the v<next> work:
   ```bash
   ts=$(date +%s)
   git checkout -b kmail-review/v<next>-$ts
   ```
3. For each "accept" or "accept with change" comment, edit the relevant
   ...
   ```
   You can apply them at the end with `git rebase --autosquash` against
   the base of the series. Do not `git push`. Do not `git rebase -i`
...
4. Run a sanity build if feasible (`make -C ... -j` on a single target
   ...
   on the new patches).
5. Regenerate the patch series into `./kmail-review/v<next>-patches/`:
   ```bash
   mkdir -p ./kmail-review/v<next>-patches
   git format-patch --cover-letter -o ./kmail-review/v<next>-patches <base>..HEAD
   ```
   Where `<base>` is the merge-base used in Phase 0 by the orchestrator
   (available in `./kmail-review/context/` metadata or recomputed from
   the original range).
...
Write `./kmail-review/context/revision-summary.json` with the
...
v<next> is ready at `./kmail-review/v<next>-patches/` and the revision
summary is at `./kmail-review/context/revision-summary.json`. The
orchestrator will present the changes to the user for approval.

Do NOT message maintainers or Linus directly — the orchestrator handles
...
- Never `git send-email`, `b4 send`, or otherwise transmit patches. They
  stay as files under `./kmail-review/v<next>-patches/`. See the HARD
  RULES section above.
