...
- Map functions modified: [function name] - [map type] - [operation: copy/update/lookup]
- copy_map_value()/copy_map_value_long() calls: [function] - [direction: kernel->user or user->kernel]
- BTF field support: [map type] - [supports special fields: yes/no] - [evidence]
- Required calls present: [check_and_init_map_value: yes/no] - [bpf_obj_free_fields: yes/no]
...
**Step 5**: Check for false positive conditions
  ...
  - Document in TodoWrite: [why this is/isn't a bug] - [evidence from map_check_btf]

**Mandatory Self-verification gate:**

**Pattern-specific questions**:
  1. How many map operations with copy_map_value() are modified or added? [number]
  2. How many of these map types can have special BTF fields? [number]
  3. How many of these map types CANNOT have special BTF fields? [number]
  4. How many copy operations need check_and_init_map_value()? [number]
  5. How many update operations need bpf_obj_free_fields()? [number]
  6. How many missing calls are actual bugs vs false positives? [bugs: number, false positives: number]
  7. Did you verify the allowlist in map_check_btf() for EACH map type? [yes/no]

If you cannot answer ALL questions with evidence, RESTART BPF-001 from the beginning.
